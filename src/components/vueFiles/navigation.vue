  ...
  <header class=" shadow-lg">
    <nav class="container flex flex-col sm:flex-row item-center gap-4 text-white py-6">
      ...
      <div class="flex gap-3 flex-1 justify-end">
        ...
        <p  v-if="isLoggedIn!=true"  @click="showRegistrationPopup" class=" font-bold text-xl hover:text-weather-secondary cursor-pointer duration-300" >SIGN UP</p>
        <p  v-if="isLoggedIn!=true" @click="showLogInPopup" class=" font-bold text-xl hover:text-weather-secondary rounded-xl px-2  bg-teal-500 cursor-pointer duration-300" >LOG IN</p>

        <div @click="toggleDashboard" v-if="isLoggedIn" class="flex bg-weather-secondary cursor-pointer  hover:bg-slate-700 items-center justify-center w-[40px] rounded-[50%] h-[40px] mt-[-5px]">{{ userInfo[0]?.username[0] }}</div>
      </div>
